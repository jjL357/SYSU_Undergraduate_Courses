...
class Area:
  ...
  def get_num_ever_fired(self):
    if self.explicit:
      # print("fghbhjgskdgd",self.num_first_winnersver)
  ...
  def project_into(self, target_area, from_stimuli, from_areas, verbose=0):
    ...
    if target_area.fixed_assembly:
        ...
        if not target_area.explicit:
            ...
            potential_new_winner_inputs = binom.rvs(total_k, self.p, size=target_area.k)
        ...
        if not target_area.explicit:
            ...
            for i in range(target_area.k):
                if new_winner_indices[i] >= target_area.w:
                    first_winner_inputs.append(all_potential_winner_inputs[new_winner_indices[i]])
